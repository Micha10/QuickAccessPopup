...
this["GuiHotkeysHelpText"] := "KEYBOARD SHORTCUTS IN SETTINGS`n`nDown: Select next item`nUp: Select previous item`n`nCtrl-N: Add a new item`nCtrl-E: Edit selected item`nCtrl-M: Move an item`nCtrl-C: Copy an item`nDel: Remove item(s)`n`nShift-Up/Down: Select contiguous items`nCtrl-Click: Select non-contiguous items`nCtrl-A: Select all items`n`nCtrl-Down: Move item(s) down`nCtrl-Up: Move item(s) up`n`nCtrl-Right: Open submenu or edit favorite`nCtrl-Left: Open parent menu"
this["GuiHotkeysHelpText2"] := "Ctrl-F: Select the Search box`nCtrl-O: Open Options window`n`nF1: Display the Help window`nCtrl-H: Display this window"
