...
this["GuiHotkeysHelpText"] := "Down: Select next favorite`nUp: Select previous favorite`n`nCtrl-N: Add a new favorite`nCtrl-M: Move a favorite`nCtrl-C: Copy a favorite`nDel: Remove favorite(s)`n`nShift-Up/Down: Select contiguous favorites`nCtrl-Click: Select non-contiguous favorites`nCtrl-A: Select all favorites`n`nCtrl-Down: Move favorite(s) down`nCtrl-Up: Move favorite(s) up`n`nCtrl-Right: Open selected submenu or group`nCtrl-Right: Edit selected favorite`nCtrl-Left: Open parent menu"
this["GuiHotkeysHelpText2"] := "F1: Display the Help window`nCtrl-F: Select the Search box`nCtrl-E: Select Extended Search`nCtrl-H: Display this window"
