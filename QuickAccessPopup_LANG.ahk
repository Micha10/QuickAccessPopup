...
global lDialogChangeHotkeyLeftAnyRight := "Each keyboard modifier can be bound to only ""Left"" key, only ""Right"" key or to ""Any"" of them."
...
global lDialogChangeHotkeyNoteSnippet := "Note: All snippet named ""~1~"" with the same content will be linked to the new hotkey." ; new
...
global lDialogFavoriteTypesHelp1 := "Create a shortcut to a frequently used ""Folder"".`n`nSet the favorite's menu name, path and various menu options.`n`nLive Folder Options allow to refresh the content of the menu with the current content of the folder.`n`nAdvanced settings allow to set the window position and file manager options." ; new
...
global lGuiDropFilesIncentive2 := "To help you dragging one or multiple files, you can make the Settings window stay ""always on top"" by clicking the pin icon on the left side of the window."
...
global lHelpText51 := "Shared menus allow to manage favorites in a setting file that can be shared by different users or used by the same user on different computers. There are three types of Shared menus:"
